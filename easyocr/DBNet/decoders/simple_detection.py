...
class SimpleEASTDecoder(SimpleDetectionDecoder):
    def __init__(
        self, feature_channels=256, densebox_ratio=1000.0, densebox_rescale_factor=512
    ):
    ...
    def calculate_losses(self, pred, label):
        ...
        densebox_loss = (densebox_loss * densebox_weight).mean(
            dim=(1, 2, 3)
        ) * self.densebox_ratio
...
class SimpleTextsnakeDecoder(SimpleDetectionDecoder):
    ...
    def calculate_losses(self, pred, label):
        ...
        radius_loss = (radius_loss * radius_weight).mean(
            dim=(1, 2, 3)
        ) * self.radius_ratio
...
class SimpleMSRDecoder(SimpleDetectionDecoder):
    def __init__(
        self, feature_channels=256, offset_ratio=1000.0, offset_rescale_factor=512
    ):
        SimpleDetectionDecoder.__init__(self, feature_channels)
    ...
    def calculate_losses(self, pred, label):
        ...
        offset_loss = (offset_loss * offset_weight).mean(
            dim=(1, 2, 3)
        ) * self.offset_ratio
