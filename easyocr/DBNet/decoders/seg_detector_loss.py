...
class SegDetectorLossBuilder:
    ...
    def build(self):
        return getattr(sys.modules[__name__], self.loss_class)(
            *self.loss_args, **self.loss_kwargs
        )
...
class L1BCEMiningLoss(nn.Module):
    ...
    def forward(self, pred, batch):
        bce_loss, bce_map = self.bce_loss(
            pred["binary"], batch["gt"], batch["mask"], return_origin=True
        )
        l1_loss, l1_metric = self.l1_loss(
            pred["thresh"], batch["thresh_map"], batch["thresh_mask"]
        )
        ...
        dice_loss = self.dice_loss(
            pred["thresh_binary"], batch["gt"], batch["mask"], weights=bce_map + 1
        )
        metrics = dict(bce_loss=bce_loss)
        metrics["thresh_loss"] = dice_loss
        loss = dice_loss + self.l1_scale * l1_loss + bce_loss * self.bce_scale
